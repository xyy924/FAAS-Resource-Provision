...
public class APP16 {
    ...
    static {
        memSize[0] = new int[]{512,512,512,512,512,512,512,512,512,512,512,512,512,512,512,512};
//        memSize[1] = new int[]{1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024};
        memSize[1] = new int[]{2048,2048,2048,2048,2048,2048,2048,2048,2048,2048,2048,2048,2048,2048,2048,2048};
//        memSize[2] = new int[]{10240,10240,10240,10240,10240,10240,10240,10240,10240,10240,10240,10240,10240,10240,10240,10240};
        memSize[2] = new int[]{4096, 4096, 4096, 4096, 4096, 4096, 4096, 4096, 4096, 4096, 4096, 4096, 4096, 4096, 4096, 4096};
    }
...
//                String content = (numOfMemSize+1) + "云工作流结束时间："+enddate + System.lineSeparator();
//                Files.write(Paths.get(TimePath), content.getBytes(), StandardOpenOption.CREATE, StandardOpenOption.APPEND);
////
//
...
//                Monitor.getCloudFlowLogs(APPName, startRFC, endTRFC, numOfMemSize + 1);
                ...
                double avgPerfCostModelDuration = Accuracy.getAvgDurationOfPerfCostModel(APP16.memConfig, APPName,numOfMemSize + 1);
                double avgPerfCostModelCost = Accuracy.getAvgCostOfPerfCostModel(APP16.memConfig, APPName,numOfMemSize + 1);
