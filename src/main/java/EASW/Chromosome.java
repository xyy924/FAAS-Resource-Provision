...
//                memConfig.put(vertices[i], ThreadLocalRandom.current().nextInt(256, 4097));
                //原作者没有的 为了满足64的步长
                memConfig.put(vertices[i], (int) (Math.round(ThreadLocalRandom.current().nextInt(256, 4097) / 64.0) * 64));
        }
    }
    ...
    public int Polynomial_mutation(WVertex vertex, int y, int left, int right, double constraint, EASW_SAL_OPT EASW_sal_opt) {
        ...
        for (; num < 10; num++) {
            ...
            //原作者没有的 为了满足64的步长
            newMem = (int) (Math.round(newMem / 64.0) * 64);
