...
public class DataStoreTools {
    public static void EASWDataStore(ArrayList<EASW_Result> results, EASW_SAL_OPT EASW_sal_opt, int iter) {
        String filenameprefix = "/src/main/resources/opt_curve_data/" + iter + "/EASW/NEW" + EASW_sal_opt.getPerfOpt().getApp().getGraph().getNode_num();
    ...
    public static void GADataStore(ArrayList<GA_Result> results, GA_SAL_OPT GA_sal_opt, int iter) {
        String filenameprefix = "/src/main/resources/opt_curve_data/" + iter + "/GA/NEW" + GA_sal_opt.getPerfOptCpu().getApp().getGraph().getNode_num();
        FileOutputStream out = null;
        try {
            File file = new File("");
            String courseFile = file.getCanonicalPath();
            filenameprefix = courseFile + filenameprefix;
            if (GA.Chromosome.getOPTType().equals("BCPO"))
                file = new File(filenameprefix + "_GA_BCPO.xls");
            else if (GA.Chromosome.getOPTType().equals("PCCO"))
                file = new File(filenameprefix + "_GA_PCCO.xls");
            out = new FileOutputStream(file);
        } catch (IOException e) {
            e.printStackTrace(System.out);
            System.exit(1);
        ...
        HSSFWorkbook workbook = new HSSFWorkbook();
        HSSFSheet sheet = workbook.createSheet("GA_results");
        HSSFRow row0 = sheet.createRow(0);
        if (GA.Chromosome.getOPTType().equals("BCPO")) {
            row0.createCell(0).setCellValue("numberOfGenes");
            row0.createCell(1).setCellValue("crossRate");
            ...
            row0.createCell(8).setCellValue("BCPO_fitness");
            row0.createCell(9).setCellValue("BCPO_opt_mem");
//            row0.createCell(10).setCellValue("BCPO_opt_cpu");
        } else if (GA.Chromosome.getOPTType().equals("PCCO")) {
            row0.createCell(0).setCellValue("numberOfGenes");
            row0.createCell(1).setCellValue("crossRate");
            row0.createCell(2).setCellValue("mutateRate");
            ...
            row0.createCell(8).setCellValue("PCCO_fitness");
            row0.createCell(9).setCellValue("PCCO_opt_mem");
//            row0.createCell(10).setCellValue("PCCO_opt_cpu");
        ...
        for (int i = 0; i < results.size(); i++) {
            ...
            row.createCell(7).setCellValue(results.get(i).getCost());
            row.createCell(8).setCellValue(results.get(i).getFitness());
            row.createCell(9).setCellValue(DataStoreTools.MemConfigToStringCpu(results.get(i).getMemConfig()));
        }
        try {
            workbook.write(out);
            out.close();
    ...
    public static void PRCPDataStore(ArrayList<ERT_C_MEM_Config_iter[]> results, int verticesNum, String OPTType, int iter) {
        String filenameprefix = "/src/main/resources/opt_curve_data/" + iter + "/PRCP/NEW" + verticesNum;
        FileOutputStream out = null;
        try {
            File file = new File("");
            String courseFile = file.getCanonicalPath();
    ...
    static String MemConfigToStringCpu(TreeMap<WVertex, MemoryCpuKey> memDict) {
        StringBuffer sb = new StringBuffer("{ ");
        for (WVertex vertex : memDict.keySet())
            sb.append(vertex.toString() + " : " + memDict.get(vertex) + " ");
        sb.append(" } ");
    ...
    public static void UWCDataStore(ArrayList<ERT_C_MEM_Config_iter> results, int verticesNum, String OPTType, int iter) {
        String filenameprefix = "/src/main/resources/opt_curve_data/" + iter + "/UWC/NEW" + verticesNum;
        FileOutputStream out = null;
        try {
            File file = new File("");
            String courseFile = file.getCanonicalPath();
    ...
    public static void DFBADataStore(ArrayList<DFBA_Result> results, int verticesNum, String OPTType, int iter) {
        String filenameprefix = "/src/main/resources/opt_curve_data/" + iter + "/DFBA/NEW" + verticesNum;
        FileOutputStream out = null;
        try {
            File file = new File("");
            String courseFile = file.getCanonicalPath();
...
//        String[] App = {"APP10", "APP16", "APP22"};
        String[] App = {"NEW10", "NEW16", "NEW21"};
//        String[] App = {"NEW21"};
        //TODO 注意这里是用的APP10 还是完整的 每次记得对应修改
        for (String OPTType : OPTTypes) {
            for (String app : App) {
                ...
                for (int iter = 1; iter < 6; iter++) {
                    ...
                    for (int row = 1; row <= rowNum; row++) {
                        ...
                        if (iter == 1) {
                            ...
                            GA.add(aRow.getCell(8).getNumericCellValue());
                        } else {
                            ...
                            GA.set(row - 1, GA.get(row - 1).doubleValue() + aRow.getCell(8).getNumericCellValue());
                        }
                    }
                }
                //TODO 这里是除循环次数 对应修改
                for (int i = 0; i < PRCP.size(); i++)
                    PRCP.set(i, PRCP.get(i).doubleValue() / 5);
                for (int i = 0; i < EASW.size(); i++)
                    EASW.set(i, EASW.get(i).doubleValue() / 5);
                for (int i = 0; i < UWC.size(); i++)
                    UWC.set(i, UWC.get(i).doubleValue() / 5);
                for (int i = 0; i < DFBA.size(); i++)
                    DFBA.set(i, DFBA.get(i).doubleValue() / 5);
                for (int i = 0; i < GA.size(); i++)
                    GA.set(i, GA.get(i).doubleValue() / 5);
                ...
                row0.createCell(5).setCellValue("GA");
                ...
                for(int i=0;i< constraints.size();i++){
                    ...
                    aRow.createCell(5).setCellValue(GA.get(i));
                }
...
//        String[] App = {"APP10"};
        for (String OPTType : OPTTypes) {
            for (String app : App) {
                ...
                ArrayList<Double> GATime = new ArrayList<>();
                //TODO 这里iter是重复次数  对应修改
                for (int iter = 1; iter < 6; iter++) {
                    ...
                    GATime.add(aRow.getCell(4).getNumericCellValue());
                }
                ...
                double avgGATime = GATime.stream().mapToDouble((x) -> x).average().getAsDouble();
                ...
                row0.createCell(1).setCellValue("PRCPExecutionTimeOf"+OPTType+"(ms)");
                ...
                row0.createCell(4).setCellValue("GAExecutionTimeOf"+OPTType+"(ms)");
                ...
                aRow.createCell(4).setCellValue(avgGATime);
