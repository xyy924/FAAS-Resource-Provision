...
public class ResultsVisualization {
    ...
    private static String[] BCPOrowKeys = {"EASW", "Without BCR", "BCR RT/M", "BCR ERT/C", "BCR MAX", "UWC", "DFBA","GA"};
    private static String[] PCCOrowKeys = {"EASW", "Without BCR", "BCR M/RT", "BCR C/ERT", "BCR MAX", "UWC", "DFBA","GA"};
    ...
    public static JFreeChart createChart(CategoryDataset categoryDataset) {
        ...
        if (ResultsVisualization.OPTType.equals("BCPO")) {
            jfreechart = ChartFactory.createLineChart(
                    "The optimal performance under budget constraint obtained from EASW, PRCP, UWC, DFBA and GA",
            ...
            jfreechart = ChartFactory.createLineChart(
                    "The optimal cost under performance constraint obtained from  EASW, PRCP, UWC, DFBA and GA",
    ...
    public static void DrawPictures(PerfOpt perfOpt,PerfOptCpu perfOptCpu, int iter) {
        ResultsVisualization.perfOpt = perfOpt;
        ResultsVisualization.perfOptCpu = perfOptCpu;
    ...
    public static double[][] getAlgorithmResults(int iter){
        ...
        try {
            ...
            if (ResultsVisualization.OPTType.equals("BCPO")) {
                ...
                double[][] data = new double[8][rowNums];
                for (int i = 0; i <= 4; i++)
                ...
                String GAFilePath = filePathPrefix + "GA/" + perfOptCpu.getApp().getGraph().getAPPName() + "_GA_BCPO.xls";
                inputStream = new FileInputStream(GAFilePath);
                workbook = new HSSFWorkbook(inputStream);
                sheet = workbook.getSheet("GA_results");
                rowNums = sheet.getLastRowNum();
                for (int i = 1; i <= rowNums; i++) {
                    HSSFRow ARow = sheet.getRow(i);
                    data[7][i - 1] = Double.valueOf(ARow.getCell(6).toString());
                }
                ...
                for (int i = 1; i <= rowNums; i++) {
                    ...
                    if (data[7][i - 1] <= data[0][i - 1]&&data[7][i - 1] <= data[1][i - 1] && data[7][i - 1] <= data[2][i - 1] && data[7][i - 1] <= data[7][i - 1] &&
                            data[7][i - 1] <= data[7][i - 1] && data[7][i - 1] <= data[7][i - 1] && data[7][i - 1] <= data[7][i - 1])
                ...
                head.createCell(8).setCellValue("GA");
                ...
                for(int i=0;i<rowNums;i++){
                    ...
                    aRow.createCell(8).setCellValue(data[7][i]);
                }
                ...
                String GAFilePath = filePathPrefix + "GA/" + perfOptCpu.getApp().getGraph().getAPPName() + "_GA_PCCO.xls";
                inputStream = new FileInputStream(GAFilePath);
                workbook = new HSSFWorkbook(inputStream);
                sheet = workbook.getSheet("GA_results");
                rowNums = sheet.getLastRowNum();
                for (int i = 1; i <= rowNums; i++) {
                    HSSFRow ARow = sheet.getRow(i);
                    data[7][i - 1] = Double.valueOf(ARow.getCell(7).toString());
                }
